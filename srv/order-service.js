...
module.exports = class OrderService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.before('CREATE', 'Orders', async (req) => {
      ...
      if (!client)        return req.reject(400, `Cliente ${client_ID} no existe`)
      if (!client.activo) return req.reject(400, `El cliente "${client.razonSocial}" está inactivo`)

      // Validar ítems
      if (!items || items.length === 0)
        return req.reject(400, 'El pedido debe tener al menos un ítem')

      for (const item of items) {
        if (!item.cantidad || item.cantidad <= 0)
          return req.reject(400, 'Todos los ítems deben tener cantidad mayor a 0')

        const product = await SELECT.one.from(Products).where({ ID: item.product_ID })
        if (!product)        return req.reject(400, `Producto ${item.product_ID} no existe`)
        if (!product.activo) return req.reject(400, `El producto "${product.descripcion}" está inactivo`)
    ...
    this.on('approve', async (req) => {
      ...
      if (!order)
        return req.reject(404, `Pedido ${orderID} no encontrado`)
      if (order.estado !== 'Borrador')
        return req.reject(400, `Solo se pueden aprobar pedidos en estado Borrador. Estado actual: ${order.estado}`)
    ...
    this.before('DELETE', 'Orders', async (req) => {
      ...
      if (!order)
        return req.reject(404, 'Pedido no encontrado')
      if (order.estado !== 'Borrador')
        return req.reject(409, `No se puede eliminar un pedido en estado "${order.estado}". Solo se eliminan pedidos en Borrador.`)
    })
